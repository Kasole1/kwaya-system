...
def init_db():
    with get_db() as conn:
        ...
        conn.execute('''CREATE TABLE IF NOT EXISTS live_stream_recordings (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            schedule_id INTEGER,
            title TEXT NOT NULL,
            youtube_url TEXT,
...
# ============ USERNAME GENERATION ============
def generate_username(role_name, voice=None, conn=None):
    """Generate username automatically based on role and voice"""
    from datetime import datetime
    
    role_lower = role_name.lower() if role_name else ''
    
    # For choir members (Mwanakwaya with voice)
    if role_lower == 'mwanakwaya' and voice:
        prefix = {'Soprano': 'S', 'Alto': 'A', 'Tenor': 'T', 'Bass': 'B'}.get(voice, 'X')
        number = 1
        if conn:
            try:
                count = conn.execute("SELECT COUNT(*) as c FROM wanakwaya WHERE sauti = ? AND status = 'active'", (voice,)).fetchone()
                if count and count['c']:
                    number = count['c'] + 1
            except:
                pass
        return f"{prefix}{str(number).zfill(3)}.bonifasi"
    
    # For roles with multiple people
    if role_lower in ['mwalimu', 'mfadhili', 'mlezi', 'rafiki']:
        base = role_lower
        number = 1
        if conn:
            try:
                count = conn.execute("SELECT COUNT(*) as c FROM watumiaji WHERE username LIKE ?", (f'{base}%',)).fetchone()
                if count and count['c']:
                    number = count['c'] + 1
            except:
                pass
        return f"{base}{number}.bonifasi"
    
    # For unique roles
    role_map = {
        'admin': 'admin',
        'mwenyekiti': 'mwenyekiti.bonifasi',
        'mwenyekiti_msaidizi': 'm/mwenyekiti.bonifasi',
        'mweka_hazina': 'mweka_hazina.bonifasi',
        'mhasibu': 'mhasibu.bonifasi',
        'katibu': 'katibu.bonifasi',
        'mkatibu': 'm/katibu.bonifasi',
        'mwalimu_kiongozi': 'mwalimu_kiongozi.bonifasi'
    }
    ...
    # Default
    return f"mtumiaji.{datetime.now().strftime('%Y%m%d%H%M%S')}"
...
def export_report(report_type):
    ...
    if format_type == 'excel':
        ...
        if report_type == 'ada':
            ...
            <table><thead><tr><th>Tukio</th><th>Tarehe</th><th>Jina</th><th>Sauti</th><th>Status</th><th>Penalty</th></tr></thead>
            <tbody>'''
            for row in data[:100]:
...
def export_wanakwaya():
    ...
    if format_type == 'excel':
        ...
        {''.join(f'<tr><td>{w["jina"]}</td><td>{w["sauti"]}</td><td>{w["simu"]}</td><td>{w["status"]}</td></tr>' for w in wanakwaya)}
...
def api_dashboard_widgets():
    ...
    upcoming_birthdays = [
        {'jina': 'Maria John', 'sauti': 'Soprano', 'days_until': 0},
        {'jina': 'John Mushi', 'sauti': 'Tenor', 'days_until': 2},
        {'jina': 'Anna Peter', 'sauti': 'Alto', 'days_until': 5},
        {'jina': 'Peter Massawe', 'sauti': 'Bass', 'days_until': 12},
        {'jina': 'Esther Joseph', 'sauti': 'Soprano', 'days_until': 18}
    ]
    return jsonify({'success': True, 'upcoming_birthdays': upcoming_birthdays})

# ============ LIVE STREAM SCHEDULES API ============
@app.route('/api/live/schedules')
def api_get_live_schedules():
    if 'user_id' not in session:
        return jsonify({'success': False, 'message': 'Unauthorized'}), 401
    from datetime import datetime, timedelta
    ...
    schedules = [
        {
            'id': 1,
            'title': '📺 Misa ya Jumapili - Live',
            'description': 'Jiunge nasi mtandaoni',
            'schedule_date': tomorrow,
            'schedule_time': '10:00',
            'duration': 60,
            'youtube_url': 'https://www.youtube.com/@kmb_sombetini/live'
        }
    ]
    return jsonify({'success': True, 'schedules': schedules})

# ============ LIVE STREAM VIEWERS API ============
@app.route('/api/live/viewers/<int:schedule_id>')
def api_get_live_viewers(schedule_id):
    if 'user_id' not in session:
        return jsonify({'success': False, 'message': 'Unauthorized'}), 401
    import random
    return jsonify({'success': True, 'viewer_count': random.randint(15, 65)})

# ============ LIVE STREAM RECORDINGS API ============
def api_get_live_recordings():
    if 'user_id' not in session:
        return jsonify({'success': False, 'message': 'Unauthorized'}), 401
    recordings = [
        {'id': 1, 'title': 'Misa ya Jumapili - Juni 2026', 'youtube_url': 'https://www.youtube.com/@kmb_sombetini', 'recording_date': '2026-06-01'},
        {'id': 2, 'title': 'Tamasha la Nyimbo Takatifu', 'youtube_url': 'https://www.youtube.com/@kmb_sombetini', 'recording_date': '2026-05-25'}
    ]
...
def api_create_user():
    ...
    if not full_name or not role_name or not raw_password:
        return jsonify({'success': False, 'message': 'Tafadhali jaza taarifa zote!'}), 400
    ...
    with get_db() as conn:
        ...
        if role_lower == 'mwanakwaya' and voice:
            prefix = {'Soprano': 'S', 'Alto': 'A', 'Tenor': 'T', 'Bass': 'B'}.get(voice, 'X')
            count = conn.execute("SELECT COUNT(*) as c FROM wanakwaya WHERE sauti = ? AND status = 'active'", (voice,)).fetchone()
            number = (count['c'] if count and count['c'] else 0) + 1
            username = f"{prefix}{str(number).zfill(3)}.bonifasi"
        elif role_lower in ['mwalimu', 'mfadhili', 'mlezi', 'rafiki']:
            count = conn.execute("SELECT COUNT(*) as c FROM watumiaji WHERE username LIKE ?", (f'{role_lower}%',)).fetchone()
            number = (count['c'] if count and count['c'] else 0) + 1
            username = f"{role_lower}{number}.bonifasi"
        elif role_lower == 'admin':
            username = 'admin'
        elif role_lower == 'mwenyekiti':
            username = 'mwenyekiti.bonifasi'
        elif role_lower == 'mwenyekiti_msaidizi':
            username = 'm/mwenyekiti.bonifasi'
        elif role_lower == 'mweka_hazina':
            username = 'mweka_hazina.bonifasi'
        elif role_lower == 'mhasibu':
            username = 'mhasibu.bonifasi'
        elif role_lower == 'katibu':
            username = 'katibu.bonifasi'
        elif role_lower == 'mkatibu':
            username = 'm/katibu.bonifasi'
        elif role_lower == 'mwalimu_kiongozi':
            username = 'mwalimu_kiongozi.bonifasi'
        else:
            from datetime import datetime
            username = f"mtumiaji.{datetime.now().strftime('%Y%m%d%H%M%S')}"
        ...
        if existing:
            ...
            count = conn.execute("SELECT COUNT(*) as c FROM watumiaji WHERE username LIKE ?", (f'{base}%',)).fetchone()
            new_num = (count['c'] if count and count['c'] else 0) + 1
            username = f"{base}{new_num}.bonifasi"
        
        # Map role to role_id
        role_map = {
            'admin': 1, 'mwenyekiti': 2, 'mwenyekiti_msaidizi': 2,
            'mweka_hazina': 4, 'mhasibu': 4, 'katibu': 2, 'mkatibu': 2,
            'mwalimu_kiongozi': 3, 'mwalimu': 3, 'mfadhili': 5,
            'mlezi': 5, 'rafiki': 5, 'mwanakwaya': 5
        }
        role_id = role_map.get(role_lower, 5)
        
        hashed = generate_password_hash(raw_password)
        cursor = conn.execute('''
            INSERT INTO watumiaji (username, password, full_name, role_id, is_first_login)
            VALUES (?, ?, ?, ?, 1)
        ''', (username, hashed, full_name, role_id))
        user_id = cursor.lastrowid
        
        # If choir member, add to wanakwaya
        if role_lower == 'mwanakwaya' and voice:
            existing_member = conn.execute('SELECT id FROM wanakwaya WHERE jina = ?', (full_name,)).fetchone()
            if not existing_member:
                conn.execute('''
                    INSERT INTO wanakwaya (jina, simu, sauti, anwani, status, tarehe_jiunga)
                    VALUES (?, ?, ?, ?, 'active', date('now'))
                ''', (full_name, '', voice, ''))
        
        conn.commit()
        log_activity(session['user_id'], session['username'], 'CREATE_USER', f'Created user: {username}')
        
        return jsonify({
            'success': True,
            'message': f'✅ Mtumiaji {full_name} ameongezwa!\nUsername: {username}',
            'user_id': user_id,
            'username': username
        })
...
def api_record_payment():
    ...
    with get_db() as conn:
        ...
        conn.execute('''
            INSERT INTO payment_requests (member_id, member_name, amount, payment_method, transaction_id, phone_number, month, year)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ''', (member_id, member_name, amount, payment_method, transaction_id, phone_number, month, year))
        
        # Notify admin
        admins = conn.execute('SELECT id FROM watumiaji WHERE role_id = 1').fetchall()
        for admin in admins:
            conn.execute('''
                INSERT INTO notifications (user_id, title, message, created_at)
                VALUES (?, ?, ?, CURRENT_TIMESTAMP)
            ''', (admin['id'], f'💳 Malipo ya {payment_method}', f'{member_name} ametuma malipo ya TSh {amount:,.0f} kupitia {payment_method}\nID: {transaction_id or "N/A"}'))
        
        conn.commit()
        
        return jsonify({'success': True, 'message': f'Malipo yako ya TSh {amount:,.0f} kupitia {payment_method} yamepokelewa. Mhasibu atathibitisha. Asante!'})
...
@app.route('/api/live/chat/<int:schedule_id>', methods=['GET'])
def api_get_live_chat(schedule_id):
    if 'user_id' not in session:
        return jsonify({'success': False, 'message': 'Unauthorized'}), 401
    
    with get_db() as conn:
        conn.execute('''CREATE TABLE IF NOT EXISTS live_chat_messages (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
...
@app.route('/api/live/chat/send', methods=['POST'])
def api_send_live_chat():
    if 'user_id' not in session:
        return jsonify({'success': False, 'message': 'Unauthorized'}), 401
    ...
    with get_db() as conn:
        conn.execute('''
            INSERT INTO live_chat_messages (schedule_id, user_id, username, message, reaction)
            VALUES (?, ?, ?, ?, ?)
        ''', (schedule_id, session['user_id'], session['username'], message, reaction))
        conn.commit()
        ...
        return jsonify({
            'success': True,
            'message': f'Albamu imefutwa pamoja na media {len(media_list)}',
            'deleted_media': len(media_list),
...
def delete_albamu(id):
    """Futa albamu na media zake zote"""
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        
        # Angalia kama albamu ipo
        cursor.execute("SELECT * FROM albamu WHERE id = ?", (id,))
        albamu = cursor.fetchone()
        
        if not albamu:
            conn.close()
            return jsonify({'error': 'Albamu haipo'}), 404
        
        # Pata media zote za albamu hii
        cursor.execute("SELECT * FROM media WHERE albamu_id = ?", (id,))
        media_list = cursor.fetchall()
        
        # Futa faili za media kutoka kwenye folder
        import os
        deleted_files = 0
        for media in media_list:
            file_path = os.path.join('static/uploads/media', media['file_name'])
            if os.path.exists(file_path):
                os.remove(file_path)
                deleted_files += 1
        
        # Futa media zote kutoka database
        cursor.execute("DELETE FROM media WHERE albamu_id = ?", (id,))
        
        # Futa albamu yenyewe
        cursor.execute("DELETE FROM albamu WHERE id = ?", (id,))
